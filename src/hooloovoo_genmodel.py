    ...
    while (not done_looping):
        for batch_idx  in xrange(self.n_train_batches):
            ...
            if (iteration + 1) % validation_frequency == 0:
                ...
                print('epoch %i, mb %i/%i, tcost %.5f, verror %.3f%%' % \
...
while lr > 0.00001:
    print "============ LR %f" % (lr)
    M =  M = Generic_model(n_in = (28,28), n_out = 10, data= data_digits, layerSpecs = specs, 
                   batch_size=200, rng=None, learning_rate=lr, activation=T.tanh, 
                   L1_reg=0., L2_reg=0.0001)
